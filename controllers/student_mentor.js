...
export const apiConnected = async (req, res) => {
  try {
    ...
    await res.status(200).json({
      message: `Mentor & Student API Connected Successfully! Kindly test the API in Postman`,
    });
  } catch (error) {
...
export const createMentor = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: `New Mentor created Successfully!`, data: newMentor });
  } catch (error) {
    //throw error if anything goes wrong
  ...
  try {
    ...
    for (const student_Id of student_Ids) {
      ...
      const existingMentor = await Mentor.findOne({
        assigned_students_id: student_Id,
      });
      if (existingMentor) {
        // Remove the student Id from the mentor's assigned_students_id array
        existingMentor.assigned_students_id =
          existingMentor.assigned_students_id.filter(
...
export const getStudentsByMentorID = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      students: mentor.assigned_students_id,
    });
  } catch (error) {
    //throw error if anything goes wrong
...
export const getMentorByStudentId = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: "Data Fetched Successfully",
      mentor: student.assigned_mentor_id,
    });
  } catch (error) {
